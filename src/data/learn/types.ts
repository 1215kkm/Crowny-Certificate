...
export interface BuildStep {
  ...
  /**
   * 명령어(설치) 스텝 맨 앞에 뜨는 「준비물」 안내.
   * "설치 없이 브라우저에서 흉내 낸다"는 안심 문구 + 진짜 컴퓨터에서 하려는
   * 사람을 위한 접이식(펼쳐보기) 안내. 읽기 전용이라 →·⌘ 같은 기호를 써도 된다.
   */
  prereq?: {
    /** 겁 안 주는 안심 한 줄 — 아무것도 설치 안 해도 된다 */
    reassure: string;
    /** 접이식 제목 (예: "진짜 내 컴퓨터에서 하려면?") */
    moreTitle: string;
    /** 펼치면 나오는 항목들 */
    more: { label: string; body: string }[];
  };
  /**
